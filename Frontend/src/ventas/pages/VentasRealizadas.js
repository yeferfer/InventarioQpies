...
const VentasRealizadas = ({ ventas }) => {
  const [ventasGestion, setVentasGestion] = useState([...ventas]);

  const deleteVentas = (event) => {
    const id = event.target.id;
    api.ventas.delete(id);
    const newVentas = ventasGestion.filter((ventas) => ventas._id !== id);
    setVentasGestion([...newVentas]);
  };
  ...
  return (
    <div>
      ...
      <Container>
        <Row className="mb-3">
          <Col xs={4}>
            <Busqueda ventas={ventas} setVentas={setVentasGestion} />
          </Col>
        </Row>
